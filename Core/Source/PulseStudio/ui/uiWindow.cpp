...
namespace PulseStudio {
		...
		: m_name(name)
	{
		...
		{
			PS_CORE_INFO("Use dark ui theme.");
			SetStyle(true); // Dark
		}
		...
		{
			PS_CORE_INFO("Use light ui theme.");
		...
		if (!fontLoaded)
		{
			TextRenderer::Get().LoadFont("H:/Projects/CppProject/Pulse-Studio/Resources/Fonts/CascadiaMono-LightItalic.ttf", 20.0f);
			fontLoaded = true;
		}
	}
	...
	{
		PS_CORE_INFO("uiWindow attached.");
	}

	void uiWindow::OnDetach()
	{
		PS_CORE_INFO("uiWindow detached.");
	}
	...
	void uiWindow::OnUpdate(float deltaTime)
	{
		...
		glColor4f(((m_Color[0] + 0.01f) > 1.0f ? 1.0f : (m_Color[0] + 0.01f)), ((m_Color[1] + 0.01f) > 1.0f ? 1.0f : (m_Color[1] + 0.01f)), ((m_Color[2] + 0.015f) > 1.0f ? 1.0f : (m_Color[2] + 0.015f)), 1.0f);
		glBegin(GL_QUADS);
		glVertex2f(m_RectX, m_RectY);
		glVertex2f(m_RectX + m_RectWidth, m_RectY);
		glVertex2f(m_RectX + m_RectWidth, m_RectY + 30.0f);
		glVertex2f(m_RectX, m_RectY + 30.0f);
		glEnd();
		...
		if (m_IsDarkTheme)
		{
			crossR = 1.0f; crossG = 1.0f; crossB = 1.0f;
		}
		else
		{
			crossR = 0.0f; crossG = 0.0f; crossB = 0.0f;
		}
	...
	void uiWindow::OnEvent(Event& event)
	{
		if (!m_IsVisible) return;
		...
		dispatcher.Dispatch<MouseButtonPressedEvent>([this](MouseButtonPressedEvent& e)
			...
			PS_CORE_INFO(std::format("Mouse pressed at ({}, {})", mx, my));
			PS_CORE_INFO(std::format("Window rect: x = {}, y = {}, w = {}, h = {}", m_RectX, m_RectY, m_RectWidth, m_RectHeight));
			...
			float closeY = m_RectY + (30.0f - m_CloseButtonSize) / 2;
			PS_CORE_INFO(std::format("Close button rect: x = {}, y = {}, w = {}, h = {}", closeX, closeY, m_CloseButtonSize, m_CloseButtonSize));
			bool inCloseButton = (mx >= closeX && mx <= closeX + m_CloseButtonSize && my >= closeY && my <= closeY + m_CloseButtonSize);
			PS_CORE_INFO(std::format("inTitleBar = {}, inCloseButton = {}", (inTitleBar ? "true" : "false"), (inCloseButton ? "true" : "false")));
			if (inCloseButton)
			{
		...
		dispatcher.Dispatch<MouseMovedEvent>([this](MouseMovedEvent& e)
			{
				...
				if (m_IsDragging)
				{
					...
					PS_CORE_INFO(std::format("Dragging: dx = {}, dy = {}", e.GetX() - m_DragStartX, e.GetY() - m_DragStartY));
					return true;
				}
